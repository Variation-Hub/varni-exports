...
function Category() {
  ...
  useEffect(() => {
    const category = catagories.find(category => category.catagoriesId === categoryId);
  ...
  return (
    <>
      ...
      <h1 className='px-32 py-10 text-center'>{data?.description}</h1>
      {categoryId === "spices" ?
        <div className="p-4">
          <ul className={productStyle.products_contianer}>
            {data?.products_detail?.map((product, index) => (
              <Link to={`/products/${categoryId}/${product.subCatagoriesId}/${product.name}`} key={index} className={productStyle.product_name}>
                <h1>{product.name}</h1>
